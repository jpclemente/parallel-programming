...
class ReductionsSuite {
  ...
  import LineOfSight._
  @Test def `should invoke the parallel construct 30 times (15 times during upsweep and 15 times during downsweep) for an array of size 17, with threshold 1`: Unit = {
    val output = new Array[Float](17)
    parLineOfSight(Array[Float](0f, 1f, 8f, 9f, 3f, 3f, 3f, 3f, 3f, 3f, 3f, 3f, 3f, 3f, 3f, 3f, 3f), output, 1)
    assertEquals(List(0f, 1f, 4f, 4f, 4f, 4f, 4f, 4f, 4f, 4f, 4f, 4f, 4f, 4f, 4f, 4f, 4f), output.toList)
  }
  ...
  @Test def `balance should work for string of length 2 and threshold 1`(): Unit = {
    def parCheck(input: String, threshold: Int, expected: Boolean): Unit =
