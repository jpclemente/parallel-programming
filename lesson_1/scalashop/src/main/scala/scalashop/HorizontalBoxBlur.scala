...
object HorizontalBoxBlur extends HorizontalBoxBlurInterface {
  ...
  def parBlur(src: Img, dst: Img, numTasks: Int, radius: Int): Unit = {
    val rowsPerTask = Math.max(src.height/numTasks, 1)
    val rows = 0 to src.height by rowsPerTask
    rows.zip(rows.tail).foreach { case(from, to) =>
      val t = task {
