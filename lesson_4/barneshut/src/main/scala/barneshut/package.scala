...
package object barneshut {
  ...
  case class Fork(
    ...
    val massX: Float =
      if (mass==0) centerX
      else (nw.massX*nw.mass + ne.massX*ne.mass + sw.massX*sw.mass + se.massX*se.mass)/(nw.mass + ne.mass + sw.mass + se.mass)
    val massY: Float =
      if (mass==0) centerY
      else (nw.massY*nw.mass + ne.massY*ne.mass + sw.massY*sw.mass + se.massY*se.mass)/(nw.mass + ne.mass + sw.mass + se.mass)
  ...
  extends Quad {
    ...
    def insert(b: Body): Quad = {
      if (size > minimumSize){
        ...
        (b+:bodies).foldRight(empty)((b, q) => q.insert(b))
      }
