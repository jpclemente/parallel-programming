...
import FloatOps._
  ...
  @Test def `Merge boundaries`(): Unit = {
    ...
    val b = sim.mergeBoundaries(b1, b2)
    assert(b.maxX == 7f, s"${b.maxX} should be 7f")
    assert(b.maxY == 7f, s"${b.maxY} should be 7f")
    assert(b.minY == 0f, s"${b.minY} should be 0f")
    assert(b.minY == 0f, s"${b.minY} should be 0f")
  }
}
